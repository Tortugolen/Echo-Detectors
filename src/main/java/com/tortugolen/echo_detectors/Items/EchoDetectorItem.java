...
public class EchoDetectorItem extends MetalDetectorItem {
    public EchoDetectorItem(Properties pProperties, int range) {
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (!pLevel.isClientSide() && pPlayer.isShiftKeyDown()) {
            if (pPlayer instanceof ServerPlayer serverPlayer) {
                NetworkHooks.openScreen(serverPlayer, new SimpleMenuProvider((pId, pInventory, ppPlayer) -> new EchoDetectorMenu(pId, pInventory, pStack, new SimpleContainerData(2)), Component.translatable("item.echo_detectors.echo_detector")), pBuffer -> pBuffer.writeItem(pStack));
            }
